...
class Shoreditch_Theme_Plugin_Enhancements {
	...
	function __construct() {
		...
		$this->plugins = array(
			array(
				...
				'message' => sprintf(
					esc_html__( 'The %1$s is necessary to use some of this theme&rsquo;s features, including: ', 'shoreditch' ),
					'<strong>' . esc_html__( 'Jetpack plugin', 'shoreditch' ) . '</strong>' ),
	...
	function get_theme_dependencies() {
		...
		if ( current_theme_supports( 'site-logo' ) ) :
			$dependencies['logo'] = array(
				'name' => esc_html__( 'Site Logo', 'shoreditch' ),
				'slug' => 'site-logo',
				'url'  => '',
				'module' => 'none',
			);
		...
		if ( current_theme_supports( 'featured-content' ) ) :
			$dependencies['featured-content'] = array(
				'name' => esc_html__( 'Featured Content', 'shoreditch' ),
		...
		if ( current_theme_supports( 'jetpack-social-menu' ) ) :
			$dependencies['social-menu'] = array(
				'name' => esc_html__( 'Social Menu', 'shoreditch' ),
		...
		if ( current_theme_supports( 'nova_menu_item' ) ) :
			$dependencies['menus'] = array(
				'name' => esc_html__( 'Menus', 'shoreditch' ),
				'slug' => 'nova_menu_item',
				'url'  => '',
		...
		if ( current_theme_supports( 'jetpack-comic' ) ) :
			$dependencies['comics'] = array(
				'name' => esc_html__( 'Comics', 'shoreditch' ),
				'slug' => 'jetpack-comic',
				'url'  => '',
				'module' => 'custom-content-types',
			);
		...
		if ( current_theme_supports( 'jetpack-testimonial' ) ) :
			$dependencies['testimonials'] = array(
				'name' => esc_html__( 'Testimonials', 'shoreditch' ),
		...
		if ( current_theme_supports( 'jetpack-portfolio' ) ) :
			$dependencies['portfolios'] = array(
				'name' => esc_html__( 'Portfolios', 'shoreditch' ),
	...
	function get_module_name( $module ) {
		$module_names = array(
			'none'                 => esc_html__( 'no specific module needed', 'shoreditch' ),
			'custom-content-types' => esc_html__( 'Custom Content Types module', 'shoreditch' ),
		);
	...
	function admin_notices() {
		...
		if ( get_user_meta( get_current_user_id(), 'shoreditch_jetpack_admin_notice', true ) === 'dismissed' ) {
			return;
		}
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' Please activate %1$s. %2$s', 'shoreditch' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . esc_html__( 'Activate', 'shoreditch' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' Please install %1$s. %2$s', 'shoreditch' ),
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . esc_html__( 'Install', 'shoreditch' ) . '</a>' : ''
				);
			}
		...
		foreach ( $this->unactivated_modules as $module => $features ) :
			...
			$notice .= '<p>';
			$notice .= sprintf(
				esc_html__( 'To use %1$s, please activate the Jetpack plugin&rsquo;s %2$s.', 'shoreditch' ),
...
function enqueue_scripts() {
	...
	if ( is_admin() && get_user_meta( get_current_user_id(), 'shoreditch_jetpack_admin_notice', true ) !== 'dismissed' ) {

		// Adds our JS file to the queue that WordPress will load
		wp_enqueue_script( 'shoreditch_jetpack_admin_script', get_template_directory_uri() . '/inc/plugin-enhancements.js', array( 'jquery' ), '20160624', true );

		// Make some data available to our JS file
		wp_localize_script( 'shoreditch_jetpack_admin_script', 'shoreditch_jetpack_admin', array(
			'shoreditch_jetpack_admin_nonce' => wp_create_nonce( 'shoreditch_jetpack_admin_nonce' ),
		));
	}
}
...
function dismiss_admin_notice() {
	...
	if ( ! isset( $_POST['shoreditch_jetpack_admin_nonce'] ) || ! wp_verify_nonce( $_POST['shoreditch_jetpack_admin_nonce'], 'shoreditch_jetpack_admin_nonce' ) ) {
		wp_die( __( 'Your request failed permission check.', 'shoreditch' ) );
	}
	// Store the user's dimissal so that the notice doesn't show again
	update_user_meta( get_current_user_id(), 'shoreditch_jetpack_admin_notice', 'dismissed' );
	// Send success message
	wp_send_json( array(
		'status' => 'success',
		'message' => __( 'Your request was processed. See ya!', 'shoreditch' )
	) );
}
add_action( 'wp_ajax_shoreditch_jetpack_admin_notice', 'dismiss_admin_notice' );
